...
//Left Menu depends on Account ID	      
	  ...
	  $data1['user_details'] = $this->Common_model->all_record('sg_user_auth',$where);
	  ...
	  $value['subscriptiontype'] = $data['user_details'] -> subscriptiontype;
	   $value['otp_option'] = $data1['user_details'] -> otp_option;
	...
	public function user_edit()
	{
			...
			$phone= $this->input->post('phone');
			  ...
			  if( $this->input->post('otp_option')==true)
			  {
				  $checked=1;
			  }
			  else{
		...
		$where1 = array('id' => $this->session->userdata("user_id"));
	    $data1 = array('otp_option' =>  $checked );
		$data1['user_details'] = $this->Common_model->record_update('sg_user_auth',$where1,$data1);
